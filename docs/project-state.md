...
| ST-ACTORS | Actor transforms recovered with stable identity across ticks | partial | Moving actor transforms are located by value in `docs/render-state.md`: 31 distinct orthonormal 3x4s across 93 draws of one shader, changing at every frame boundary, in a capture at frame 900. Identity is the open half and one approach is now ruled out by measurement: draw-index matching is 28x worse than nearest-neighbour (mean 296 against 10.39 units between frames) because draw counts fall 93->69 as objects enter and leave. Nearest neighbour is rejected as a heuristic that swaps identity when actors pass close. Exact gap: the capture records the assembled buffer but not the guest address backing it, so the engine's own object storage cannot be used as the key yet. |
