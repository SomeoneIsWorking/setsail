...
| ST-PERF | 60 Hz sustained, frame-time percentiles published with the tested hardware | partial | Measured walking with interpolation on (`wiiuport` `tools/continuous_run.py`): frame time p50 19.7 ms, p95 40.7 ms, p99 48.6 ms over 1,310 intervals with the vertex blend (p50 16.8, p95 34.0 ms without it) -- not sustained, since the halves of a tick are uneven (median 31.2 ms title-to-in-between, 13.1 ms back) and the in-between frame's replay waits 4.5 ms a tick for vertex blending. The tested hardware is not yet published with it. |
