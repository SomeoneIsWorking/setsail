...
| ST-ACTORS | Actor transforms recovered with stable identity across ticks | partial | Candidate located in `docs/render-state.md`: uniform-block location 4 carries 768-byte blocks (192 floats, the shape of a 16-entry 3x4 skinning palette) across 58 distinct source buffers in 12,510 calls -- the per-object signature, as against the 2-buffer engine-owned blocks at location 3. Gaps: no values captured, and no cross-tick identity yet, which is the harder half -- a buffer pointer is not an actor. |
